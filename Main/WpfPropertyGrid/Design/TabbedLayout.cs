...
namespace System.Windows.Controls.WpfPropertyGrid.Design
{
	...
	public class TabbedLayout : TabControl
	{
		...
		{
			TabbedLayoutItem layoutItem = SelectedItem as TabbedLayoutItem;
			if (layoutItem != null)
				e.CanExecute = layoutItem.CanClose;
			else
			{
				DependencyObject obj = SelectedItem as DependencyObject;
				if (obj != null)
		...
		{
			PropertyItemValue value = e.Parameter as PropertyItemValue;
			if (value == null) return;
			PropertyItem property = value.ParentProperty;
			if (property == null) return;

			// Try getting already opened extended tab
			TabbedLayoutItem extendedTab = FindExtendedEditorTab(property);
			if (extendedTab != null)
			{
		...
		protected override void PrepareContainerForItemOverride(DependencyObject element, object item)
		{
			...
			TabbedLayoutItem tab = element as TabbedLayoutItem;
			if (tab != null)
			{
				//TODO: Assign PG as DataContext here?
				//tab.DataContext = item;

				DependencyObject layout = item as DependencyObject;
				if (layout != null)
				{
				...
				{
					Binding bHeader = new Binding(ItemHeaderProperty)
					{
						Source = item,
		...
		{
			TabbedLayoutItem tabItem = sender as TabbedLayoutItem;
			...
			{
				GridEntry visibleEntry = GetFirstVisibleEntry();
				if (SelectedItem != null && SelectedItem != visibleEntry)
