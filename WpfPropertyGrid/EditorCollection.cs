...
namespace System.Windows.Controls.WpfPropertyGrid
{
	...
	public class EditorCollection : Collection<Editor>
	{
		...
		public CategoryEditor FindCategoryEditor(Type declaringType, string categoryName)
		{
			...
			return this.OfType<CategoryEditor>().Where(item => item.DeclaringType.IsAssignableFrom(declaringType)).FirstOrDefault(item => item.CategoryName == categoryName);
		}
		...
		public static Editor GetPropertyEditorByAttributes(AttributeCollection attributes)
		{
			...
			PropertyEditorAttribute attribute = attributes[KnownTypes.Attributes.PropertyEditorAttribute] as PropertyEditorAttribute;
		...
		public Editor GetEditor(PropertyItem propertyItem)
		{
			...
			if (hasType)
			{
				foreach (KeyValuePair<Type, Editor> cachedEditor in cache)
